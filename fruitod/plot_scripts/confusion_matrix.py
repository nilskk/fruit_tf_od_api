...
tf.compat.v1.flags.DEFINE_string('input_tfrecord_path', './data/tfrecords/vott_val.tfrecord',
...
tf.compat.v1.flags.DEFINE_string('class_labels', './data/voc_data/pascal_label_map.pbtxt',
    ...
    for image_index, record in enumerate(input_dataset):
        ...
        if decoded_dict:
            ...
            detection_classes = detections['detection_classes'][0].numpy()[
                detection_scores >= CONFIDENCE_THRESHOLD].astype('uint8')
...
def display(confusion_matrix, categories, output_path):
    ...
    name_list[len(categories)] = 'Background'

    df = pd.DataFrame(confusion_matrix.astype(np.int64))
    df = df.rename(lambda x: name_list[x], axis=0)
    df = df.rename(lambda x: name_list[x], axis=1)
    df = df.rename_axis('Prediction', axis=0)
    df = df.rename_axis('Groundtruth', axis=1)
    print(df)
    df.to_csv(output_path)


def draw(image_name, image_path, image, categories, groundtruth_boxes, groundtruth_classes, detection_boxes,
...
def main(_):
    ...
    # Get class names
    categories = label_map_util.create_category_index_from_labelmap(FLAGS.class_labels, use_display_name=True)
